"""
Reference two-impulse Δv formulas from the project objective (CLAUDE.md).

Used in two places:
  - libs/env.py    : sizes the *soft* Δv ceiling used for early-truncating
                      hopeless/runaway episodes (not a hard action clip).
  - eval scripts    : reported alongside the trained policy's actual Δv so
                      the learned solution can be judged against the
                      classical analytic transfer.

All formulas take Δx / Δz as positive magnitudes.
"""

import numpy as np


def dv_vbar_two_impulse(dx: float, omega: float) -> float:
    """Two V-bar impulses, Δx displacement along V-bar (goal 1)."""
    return omega / (3.0 * np.pi) * abs(dx)


def dv_rbar_two_impulse(dz: float, omega: float) -> float:
    """Two R-bar impulses, Δx displacement along V-bar (goal 1 comparison)."""
    return omega / 2.0 * abs(dz)


def dv_rbar_strategy_rv(dz: float, omega: float) -> float:
    """R-bar impulse + V-bar impulse (goal 2, strategy 1). Δv_tot = 3·ω·Δz."""
    return 3.0 * omega * abs(dz)


def dv_rbar_strategy_vv(dz: float, omega: float) -> float:
    """Two V-bar impulses (goal 2, strategy 2). Δv_tot = (ω/2)·Δz."""
    return omega / 2.0 * abs(dz)


def reference_dv_for_scenario(scenario: str, dx: float, dz: float, omega: float) -> float:
    """
    Best-case (smallest) classical reference Δv for a given scenario and
    displacement, used only to size a *generous* soft training ceiling.
    """
    if scenario == "vbar":
        return dv_vbar_two_impulse(dx, omega)
    if scenario == "rbar":
        return min(
            dv_rbar_strategy_rv(dz, omega),
            dv_rbar_strategy_vv(dz, omega),
        )
    raise ValueError(f"Unknown scenario: {scenario!r}")
